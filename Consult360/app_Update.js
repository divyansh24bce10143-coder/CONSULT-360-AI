...
async function processFile(file) {
  ...
  try {
    ...
    if (!text.trim()) {
      status.innerHTML = `<p style="color:var(--danger)">⚠ Could not extract text. Try a text-based PDF.</p>`;
      return;
    }
...
async function callGemini(prompt) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1/models/gemini-3.6-flash:generateContent?key=${state.apiKey}`;
